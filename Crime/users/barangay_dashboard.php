<?php
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'submit_report') {
        ...
        try {
            ...
            if ($locationAddress !== '') {
                $extraLines[] = 'Address: ' . $locationAddress;
            }
...
<body class="bg-light">
            ...
            <div class="card-body">
                ...
                <form id="crime-report-form">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="mt-2">
                            <label class="form-label" for="location_address">Resolved Address</label>
                            <input type="text" class="form-control" id="location_address" name="location_address" placeholder="Address will auto-fill after map click" readonly>
                        </div>
    ...
    <script>
        ...
        async function resolveAddress(lat, lng) {
            ...
            try {
                const response = await fetch(`barangay_dashboard.php?${params.toString()}`);
                const result = await response.json();

                if (result.success && result.address) {
                    document.getElementById('location_address').value = result.address;
                    document.getElementById('location-label').textContent = `Address: ${result.address}`;
                } else {
                    document.getElementById('location_address').value = '';
                    document.getElementById('location-label').textContent = 'Address not found for this pin. You may click another location.';
                }
            } catch (error) {
                document.getElementById('location_address').value = '';
                document.getElementById('location-label').textContent = 'Unable to resolve address right now. Please try another click.';
            }
        }
        ...
        async function setLocation(lat, lng) {
            ...
            document.getElementById('location-label').textContent = 'Resolving address...';
