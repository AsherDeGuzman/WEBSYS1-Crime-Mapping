...
function renderAlerts(alerts) {
    ...
    if (!alerts.length) {
        alertsContainer.innerHTML = "<div class=\"alert-item\">No high severity alerts.</div>";
        return;
    }

    alerts.forEach((incident) => {
        const item = document.createElement("div");
        item.className = "alert-item";
        item.innerHTML = `
            <strong>${incident.title}</strong>
            <div class="alert-meta">High severity • ${incident.barangay}</div>
        `;
